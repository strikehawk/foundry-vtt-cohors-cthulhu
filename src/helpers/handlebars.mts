export function registerHandlebarsHelpers(): void {
  ...
  Handlebars.registerHelper(
    "subString",
    function (str: string, s: number, e: number | undefined) {
  ...
  Handlebars.registerHelper(
    "math",
    function (lOperand: string, operator: string, rOperand: string, options) {
      const lvalue = parseFloat(lOperand);
      const rvalue = parseFloat(rOperand);
  ...
  Handlebars.registerHelper(
    "enrichHtmlHelper",
    function (rawText: string): string {
      return TextEditor.enrichHTML(rawText, { async: false } as any); // TODO Possible Foundry VTT types issue
    }
  );
  ...
  Handlebars.registerHelper(
    "colorIfValue",
    function (num, compare, color, color2) {
